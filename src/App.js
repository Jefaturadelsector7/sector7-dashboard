...
const users = {
  ...
  "supervisor@sector7.edu.mx": { password: "Supervisor@2025", role: "supervisor", name: "Supervisor Zona 18", zona: "Zona 18", escuela: null },
...
export default function App() {
  ...
  const [escuelasForm, setEscuelasForm] = useState({
    nombre: "",
    director: "",
    supervisionId: "",
    estudiantes: "",
  ...
  const [supervisionesForm, setSupervisionesForm] = useState({
    zona: "",
    supervisor: ""
  });
  ...
  useEffect(() => {
    ...
    getDocs(query(collection(db, "supervisiones"), orderBy("zona")))
      .then((snapshot) => {
        setSupervisiones(snapshot.docs.map(d => ({ id: d.id, ...d.data() })));
      })
      .catch(err => console.error("Error cargando supervisiones:", err));
  }, [user]);

  useEffect(() => {
    if (supervisiones.length === 0 || !userRole) return;
  ...
  }, [supervisiones, userRole, userZona, userEscuela]);
  ...
  const handleEscuelasSubmit = (e) => {
    ...
    addDoc(collection(db, "escuelas"), {
      ...
      getDocs(query(collection(db, "escuelas"), orderBy("nombre")))
        .then(snapshot => setEscuelas(snapshot.docs.map(d => ({ id: d.id, ...d.data() }))));
    }).catch(err => {
      setError("Error al guardar escuela: " + err.message);
    });
  };
  ...
  const handleSupervisionesSubmit = (e) => {
    ...
    addDoc(collection(db, "supervisiones"), {
      ...
      getDocs(query(collection(db, "supervisiones"), orderBy("zona")))
        .then(snapshot => setSupervisiones(snapshot.docs.map(d => ({ id: d.id, ...d.data() }))));
    }).catch(err => {
      setError("Error al guardar supervisión: " + err.message);
    });
  };
      ...
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "100vh", background: "#1e40af" }}>
        <div style={{ background: "white", padding: "40px", borderRadius: "8px", width: "100%", maxWidth: "400px", boxShadow: "0 10px 40px rgba(0,0,0,0.2)" }}>
          ...
          <p style={{ fontSize: "11px", color: "#666" }}>
            <strong>Admin:</strong> rocio@sector7.edu.mx / Sector7@2025
          </p>
        </div>
      </div>
    );
  ...
  const userName = users[user.email]?.name || user.email;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f5f5" }}>
      <header style={{ background: "#1e40af", color: "white", padding: "20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ margin: 0 }}>SECTOR 7 - DASHBOARD</h1>
      ...
      <div style={{ padding: "40px", maxWidth: "1200px", margin: "0 auto" }}>
          ...
          <div>
            <div style={{ display: "flex", gap: "10px", marginBottom: "30px", borderBottom: "2px solid #e5e7eb", flexWrap: "wrap" }}>
              <button 
                ...
                style={{ padding: "10px 20px", background: activeTab === "escuelas" ? "#2563eb" : "transparent", color: activeTab === "escuelas" ? "white" : "#666", border: "none", cursor: "pointer", fontWeight: "600" }}
              >
                Escuelas
              </button>
              <button 
                onClick={() => setActiveTab("supervisiones")}
                style={{ padding: "10px 20px", background: activeTab === "supervisiones" ? "#2563eb" : "transparent", color: activeTab === "supervisiones" ? "white" : "#666", border: "none", cursor: "pointer", fontWeight: "600" }}
              >
                Supervisiones
              </button>
            </div>
            ...
            {activeTab === "escuelas" && (
              <div>
                <div style={{ marginBottom: "20px" }}>
                  <button onClick={() => setShowEscuelasForm(!showEscuelasForm)} style={{ padding: "10px 20px", background: "#2563eb", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "600" }}>
                    ...
                    <form onSubmit={handleEscuelasSubmit} style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                      ...
                      <select name="supervisionId" value={escuelasForm.supervisionId} onChange={handleEscuelasChange} style={{ padding: "10px", border: "1px solid #ddd", borderRadius: "4px" }} required>
                        ...
                        {supervisiones.map((supervision) => (
                          <option key={supervision.id} value={supervision.id}>
                            {supervision.zona}
                          </option>
                        ))}
                      </select>
                ...
                <div style={{ background: "white", padding: "20px", borderRadius: "8px" }}>
                  ...
                  {escuelas.length === 0 ? (
                    <p style={{ color: "#666" }}>No hay escuelas registradas</p>
                  ) : (
            ...
            {activeTab === "supervisiones" && (
              <div>
                <div style={{ marginBottom: "20px" }}>
                  <button onClick={() => setShowSupervisionesForm(!showSupervisionesForm)} style={{ padding: "10px 20px", background: "#2563eb", color: "white", border: "none", borderRadius: "4px", cursor: "pointer", fontWeight: "600" }}>
                ...
                <div style={{ background: "white", padding: "20px", borderRadius: "8px" }}>
                  ...
                  {supervisiones.length === 0 ? (
                    <p style={{ color: "#666" }}>No hay supervisiones registradas</p>
                  ) : (
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
                      <thead>
                        <tr style={{ background: "#f3f4f6", borderBottom: "2px solid #e5e7eb" }}>
        ...
        {userRole !== "admin" && (
          <div>
            <h2 style={{ color: "#1e40af" }}>Acceso restringido</h2>
